            ...
            <div x-show="open" class="fixed inset-0 flex items-center justify-center z-50" style="display:none;">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 z-50 w-full max-w-md">
                    ...
                    <form action="#" method="POST">
                        ...
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Customer Password</label>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-700">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-200 uppercase tracking-wider">KYC Status</th>
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ $customer->id }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ $customer->name }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ $customer->email }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            @if(!$customer->document)
                                <span class="px-2 py-1 bg-gray-400 text-white rounded">No document</span>
                            
                            @elseif($customer->document->status == 'pending')
                                <span class="px-2 py-1 bg-yellow-400 text-black rounded">Pending</span>

                            @elseif($customer->document->status == 'approved')
                                <span class="px-2 py-1 bg-green-500 text-white rounded">Approved</span>

                            @elseif($customer->document->status == 'rejected')
                                <span class="px-2 py-1 bg-red-500 text-white rounded">Rejected</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ $customer->created_at->format('d-m-Y - H:i:s') }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <a href="{{ route('admin.customer.show', $customer->id) }}" class="px-2 py-1 bg-blue-500 hover:bg-blue-600 text-white rounded-md text-md mr-2 font-bold">View</a>
                            <!-- Edit Button -->
                            <button @click="editId={{ $customer->id }}; editName='{{ $customer->name }}'; editOpen=true;"
