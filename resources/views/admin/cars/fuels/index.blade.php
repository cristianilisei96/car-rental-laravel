<x-admin.layout>
    <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        <!-- Header și Buton Add New fuel -->
        <div class="flex justify-between items-center mb-6" x-data="{ open:false }">
            <h1 class="text-2xl font-semibold text-gray-900 dark:text-gray-100">Car Fuel</h1>
            ...
            <div x-show="open" class="fixed inset-0 flex items-center justify-center z-50" style="display:none;">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 z-50 w-full max-w-md">
                    <h2 class="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-4">Add New Fuel</h2>
                    <form action="{{ route('fuels.store') }}" method="POST">
                        @csrf
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Fuel Name</label>
        ...
        <div x-data="{ editOpen:false, editId:null, editName:'' }" class="overflow-x-auto bg-white dark:bg-gray-800 shadow rounded-lg">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ $fuel->id }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ $fuel->name }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ $fuel->created_at }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <!-- Edit Button -->
                            <button @click="editId={{ $fuel->id }}; editName='{{ $fuel->name }}'; editOpen=true;"
                                    class="px-2 py-1 bg-yellow-400 hover:bg-yellow-500 text-white rounded-md text-md mr-2 font-bold">
                                Edit
                            </button>
                            <!-- Delete Form -->
                            <form action="{{ route('fuels.destroy', $fuel->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                        class="px-2 py-1 bg-red-600 hover:bg-red-700 text-white rounded-md text-md"
                                        onclick="return confirm('Are you sure you want to delete this fuel?')">
                                    Delete
                                </button>
                            </form>
                        </td>
            ...
            <div x-show="editOpen" class="fixed inset-0 flex items-center justify-center z-50" style="display:none;">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 z-50 w-full max-w-md">
                    <h2 class="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-4">Edit Fuel</h2>
                    <form :action="`/admin/cars/fuels/${editId}`" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Fuel Name</label>
