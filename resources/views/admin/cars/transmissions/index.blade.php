            ...
            <div x-show="open" class="fixed inset-0 flex items-center justify-center z-50" style="display:none;">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 z-50 w-full max-w-md">
                    ...
                    <form action="{{ route('admin.transmissions.store') }}" method="POST">
                        @csrf
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Transmission
                                Name</label>
                    ...
                    @foreach ($transmissions as $transmission)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                {{ $transmission->id }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                {{ $transmission->name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                {{ $transmission->created_at }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <!-- Edit Button -->
                                <button
                                    @click="editId={{ $transmission->id }}; editName='{{ $transmission->name }}'; editOpen=true;"
                                    class="px-2 py-1 bg-yellow-400 hover:bg-yellow-500 text-white rounded-md text-md mr-2 font-bold">
                                    Edit
                                </button>
                                <!-- Delete Form -->
                                <form action="{{ route('admin.transmissions.destroy', $transmission->id) }}"
